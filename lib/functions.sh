...
create_img() {
    ...
    case "$FILESYSTEM" in
        ...
        *)
            # Create the boot and root partitions
            case "$DEVICE" in
                oc2|on2|on2-plus|on2l|oc4|ohc4|vim1|vim2|vim3|vim3l|radxa-zero|radxa-zero2|gtking-pro| \
                rpi3|rpi4|rpi4-cutiepi|pinephone|g1-tpc)
                    parted -s $LDEV mklabel msdos > /dev/null 2>&1
                    parted -s $LDEV mkpart primary fat32 32M 512M > /dev/null 2>&1
                    START=`cat /sys/block/$DEV/${DEV}p1/start`
    ...
    if [[ "$DEVICE" != "generic" ]] && [[ "$DEVICE" != "generic-efi" ]]; then
        ...
        case "$DEVICE" in
            ...
            pinetab2|quartz64-a|quartz64-b|soquartz-cm4|rock3a|pinenote|edgev|station-m2|station-m3|station-p2|om1|opi4-lts|opi-800|opi5|itx-rk3588j)
